...
import { useModuleQuestions } from '@/hooks/useModuleQuestion'
import { CreateQuestionModal, DeleteQuestionModal, EditQuestionModal } from './modal'

export default function ModuleQuestions() {
	const { moduleId = "2" } = useParams<{ moduleId: string }>()
	const { openModal, closeModal } = useModal()
	const { loading, questions, fetchQuestion, handleCreate, handleUpdate, handleDelete } = useModuleQuestions(moduleId)
	...
	const handleOpenCreate = () => {
		openModal({
			...
			btnTitle: 'Yaratish',
			content: <CreateQuestionModal moduleId={moduleId} closeModal={closeModal} handleCreate={handleCreate} />,
		})
	}
