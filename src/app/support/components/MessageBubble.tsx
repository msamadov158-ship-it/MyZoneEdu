...
export const MessageBubble = ({ message, onEdit, onDelete }: { message: Message; onEdit: (msg: Message) => void; onDelete: (msg: Message) => void }) => {
	...
	return (
  <div
    className={`flex ${isOwn ? "justify-end" : "justify-start"} items-end mb-4 group`}>
    ...
    <div className="flex flex-col">
      <div
        className={`max-w-2/3 rounded-xl px-3 py-2 relative ${
          isOwn
            ? "bg-white text-dark border border-gray-400 rounded-tr-none self-end "
            : "bg-red-100 text-dark rounded-tl-none"
					}`}
				>
					{message.message && <p className="text-sm mr-5">{message.message}</p>}
					{message.file_path && (
					<MessageFileRenderer file_path={message.file_path} />
					)}
